...
public class 좌표압축_18870 {
	/*
	 * 접근: 이중 for문으로 각 원소에 대해 다른 모든 원소와 크기 비교를 하면 O(N^2)시간으로
	 ...
	 * 
	 * 시간복잡도: 정렬에서 O(NlogN), Set객체로 만들때 어느정도 시간이 걸리는지 잘 모르겠습니다.
	 */
...
//		int[] sorted= Arrays.copyOf(coordinates, N);
//		
//		Arrays.sort(sorted);
//		
...
//		int cnt=0;
//		for(int i=0;i<N;i++) {
//			if(map.get(sorted[i])==null) map.put(sorted[i], cnt++);
//		}
//		
//		
//		for(int coordinate:coordinates) {
